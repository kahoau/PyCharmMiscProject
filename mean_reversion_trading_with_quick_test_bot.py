...
MAGIC_NUMBER = 123456  # Unique ID for bot trades (isolates from manual trades)
CHECK_INTERVAL = 60  # Signal check interval (seconds)
TEST_MODE = True  # Enable test mode (force a test order immediately)
TEST_TRADE_TYPE = mt5.ORDER_TYPE_BUY  # Test order type (mt5.ORDER_TYPE_BUY = BUY LIMIT, mt5.ORDER_TYPE_SELL = SELL LIMIT)
PENDING_ORDER_OFFSET = 0.2  # Pending order offset (pips) from current bid (fixes retcode 10015)
...
def run_test_mode():
    """
    Execute test mode: Force a test pending order and return success status
    Fixes global variable syntax error by isolating test mode logic
    """
    print(
        f"\n📢 Test Mode Enabled: Forcing {('BUY LIMIT' if TEST_TRADE_TYPE == mt5.ORDER_TYPE_BUY else 'SELL LIMIT')} Test Order...")

    # Fetch minimal data for test indicators
    data = fetch_m1_data(periods=ATR_PERIOD + 10)
    if data is None:
        print("❌ Failed to fetch data for test order")
        return False

    # Calculate minimal indicators (only ATR needed for test SL/TP)
    closes = data['close']
    highs = data['high']
    lows = data['low']
    ...
    test_indicators = {
        'atr': atr[-1] if len(atr) > 0 else 10.0,  # Default ATR if calculation fails
        'last_close': closes[-1]
    }

    # Close opposite positions + cancel pending orders before test
    close_opposite_position_and_cancel_pending()

    # Place test order
    test_success = place_pending_order(TEST_TRADE_TYPE, test_indicators)

    if test_success:
        print("\n✅ Test Mode Complete | Disabling test mode for subsequent runs")
    return test_success


def main_trading_loop():
    """
    Main trading loop: Run test mode first (if enabled), then production mode
    No global variable errors (test mode is isolated to run_test_mode())
    """
    global TEST_MODE  # Declare global at start of function (fixes syntax error)

    print("\n🚀 Starting Lirunex XAUUSD.std Trading Bot")
    ...
    # Step 1: Run test mode (if enabled)
    test_passed = False
    if TEST_MODE:
        test_passed = run_test_mode()
        TEST_MODE = False  # Disable test mode after execution (no syntax error now)

    # Step 2: Switch to production mode (regardless of test success)
    print("\n🔄 Switching to Production Mode (monitoring for RSI/SMA signals)...")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("1. Test order is in MT5's 'Orders' tab (Ticket: XXXXXXXX)")
        print("2. Set TEST_MODE=False to disable test orders (production mode only)")
        print("3. To test SELL LIMIT: Change TEST_TRADE_TYPE = mt5.ORDER_TYPE_SELL")